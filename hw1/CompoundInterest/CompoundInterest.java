...
public class CompoundInterest {
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return futureValue(presentValue, rate, targetYear) * Math.pow(((100 - inflationRate)/100), numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        if (THIS_YEAR == targetYear){
            return perYear;
        } else {
            return perYear * Math.pow((1 + (rate/100)), numYears(targetYear)) + totalSavings(perYear, targetYear - 1, rate);
        }
    }


    /** Returns totalSavings(PERYEAR, TARGETYEAR, RATE) converted to
     *  current year dollars, assuming a uniform inflation rate of {16550 * 0.97}
     *  INFLATIONRATE. */
    static double totalSavingsReal(double perYear, int targetYear, double rate,
                               double inflationRate) {
        return totalSavings(perYear, targetYear, rate) * Math.pow(((100 - inflationRate)/100), numYears(targetYear));
